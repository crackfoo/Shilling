...
// Copyright (c) 2011-2012 Shilling Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ShillingTEST0\r");
            Send(hSocket, "WHO #ShillingTEST0\r");
        } else {
            // randomly join #Shilling00-#Shilling99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Shilling%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Shilling%02d\r", channel_number).c_str());
        }
